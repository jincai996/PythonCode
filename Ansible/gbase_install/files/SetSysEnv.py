...
def CheckExecUser():
    ...
    if curUserName != "root":
        logger.info("please exec SetSysEnv.py by root, but current user is %s" % curUserName)
        raise InstallError("please exec SetSysEnv.py by root.")
...
def SetSecurityLimits():
    ...
    if rc != 0 and path.exists("/etc/security/limits.conf"):
        ...
        if rc != 0 and path.exists("/etc/sysconfig/ulimit"):
...
def SetLogrotateGCLogConf():
    ...
    if path.exists(confFileDir):
        ...
        for f in ["express","system","gc_recover"]:
...
def SetLogrotateGNLogConf():
    ...
    if path.exists(confFileDir):
        ...
        for f in ["express","system","syncserver"]:
